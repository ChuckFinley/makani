...
segPerTrip %>%
  ...
  mutate(Legs = factor(Legs, 
                       ...
                       labels = c('Inbound only', 'Outbound only', 'Outbound and Inbound'))) %>%
  ggplot(aes(x = DeployID,
             y = Count,
             fill = Legs)) +
...
Pick out the acceleration data from the transit segments to plot raw values and examine flight behavior patterns.
...
accCl <- makeCluster(3)
registerDoParallel(makeCluster)
taskPackages <- c('dplyr', 'lubridate')
# Process acceleration data by deployment and segment
segmentsACC <- foreach(accMetaRow = iter(accMeta, by = 'row'),
                       .combine = rbind) %do% {
  ...
  # Gather the segments from the current deployment
  taskList <- filter(segmentsACCmeta,
                     DeployID == accMetaRow$DeployID)
  
  # Process acceleration data for each segment
  deployACC <- foreach(segment = iter(taskList, by = 'row'),
                       .combine = rbind,
                       .packages = taskPackages) %do% {
    # Acceleration sampling frequency (20hz)
    acc.freq <- acc.data$acc.freq[1]
    
    # Filter down to acceleration data from segment
    acc.data %>%
    filter(TimestampUTC >= segment$Start, TimestampUTC <= segment$End) %>%
      # Split raw acceleration data and convert to data frame
      rowwise %>% 
      do({
        cbind(.,
              strsplit(.$acc.raw, ' ') %>%
      ...
      ungroup %>%
      # Drop raw acceleration data
      select(-acc.raw) %>% 
      # Join segment data
      cbind(segment) %>%
      group_by(BurstID) %>% 
      # Calculate acceleration timestamp and calibrate acceleration data
      mutate(ACCTimestampUTC = TimestampUTC + (seq_along(X0) - 1) / acc.freq,
             ...
             dynY = Y - staticY,
             dynZ = Z - staticZ,
             # Variance of dynamic heave in a sliding window of 0.5s
