...
fe.fv.max <- FlapRatio ~ Tailwind * Crosswind
re.fv.form <- list(# Intercepts
                   ~ . + (1|DeployID),
                   ~ . + (1|Leg),
                   # Slopes
                   ~ . + (Tailwind|DeployID),
                   ~ . + (Crosswind|DeployID),
                   ~ . + (Tailwind|DeployID) + (Crosswind|DeployID),
                   ~ . + (Tailwind|Leg),
                   ~ . + (Crosswind|Leg),
                   ~ . + (Tailwind|Leg) + (Crosswind|Leg),
                   # Both
                   ~ . + (Tailwind|DeployID) + (Tailwind|Leg),
                   ~ . + (Crosswind|DeployID) + (Crosswind|Leg),
                   ~ . + (Tailwind|DeployID) + (Crosswind|Leg),
                   ~ . + (Crosswind|DeployID) + (Tailwind|Leg))
```
...
Based on AIC the top ranked FV random effects structure is (1|B). (T|B) and (C|B) had delta AIC of 2.
...
me.fv.form <- list(list(~ . + Tailwind * Crosswind, re.fv.optim),
                list(~ . + Tailwind + Crosswind, re.fv.optim),
                list(~ . + Tailwind, re.fv.optim),
...
modCl <- makeCluster(detectCores() - 1, type = 'FORK')
registerDoParallel(modCl)
me.fv.mod <- foreach(f = me.fv.form,
                     i = seq_along(re.fv.form)) %dopar% {
  # Update 
...
```{r setup_re_fs}
fe.fs.max <- FlapRatio ~ WindAngle * WindSpd
re.fs.form <- list(# Intercepts
                   ...
                   ~ . + (WindAngle|DeployID),
                   ~ . + (WindSpd|DeployID),
                   ~ . + (WindAngle|DeployID) + (WindSpd|DeployID),
                   ~ . + (WindAngle|Leg),
                   ~ . + (WindSpd|Leg),
                   ~ . + (WindAngle|Leg) + (WindSpd|Leg),
                   # Both
                   ~ . + (WindAngle|DeployID) + (WindAngle|Leg),
                   ~ . + (WindSpd|DeployID) + (WindSpd|Leg),
                   ~ . + (WindAngle|DeployID) + (WindSpd|Leg),
                   ~ . + (WindSpd|DeployID) + (WindAngle|Leg))
```
...
Based on AIC the top ranked FV random effects structure is (1|B). (M|B) and (A|B) had delta AIC of 1.978 and 2, respectively.
...
# List to store fixed effects model formulae
me.fs.form <- list(list(~ . + WindAngle * WindSpd, re.fs.optim),
                list(~ . + WindAngle + WindSpd, re.fs.optim),
                list(~ . + WindAngle, re.fs.optim),
...
modCl <- makeCluster(detectCores() - 1, type = 'FORK')
registerDoParallel(modCl)
me.fs.mod <- foreach(f = me.fs.form,
                     i = seq_along(re.fs.form)) %dopar% {
  # Update 
  mod.formula <- update.formula(f[[2]], f[[1]])
  tryCatch(glmmadmb(mod.formula, segSample, family = 'beta'), error = function(e) NULL)
}
...
```{r setup_re_ov}
fe.ov.max <- MeanODBA ~ Tailwind * Crosswind
...
Based on AIC the top ranked OV random effects structure is (T|B) + (T|L). (C|B) + (T|L) had delta AIC of 1.882.

## Determine optimal fixed effects structure
Using the optimal random effects structure, determine best fitting fixed effects structure
```{r setup_fe_ov}
# Formula of optimal random effects structure
re.ov.optim <- MeanODBA ~ (Tailwind|DeployID) + (Tailwind|Leg)
# List to store fixed effects model formulae
me.ov.form <- list(list(~ . + Tailwind * Crosswind, re.ov.optim),
                list(~ . + Tailwind + Crosswind, re.ov.optim),
                list(~ . + Tailwind, re.ov.optim))
```
...
modCl <- makeCluster(detectCores() - 1, type = 'FORK')
registerDoParallel(modCl)
me.ov.mod <- foreach(f = me.ov.form,
                     i = seq_along(re.ov.form)) %dopar% {
  # Update 
  mod.formula <- update.formula(f[[2]], f[[1]])
  tryCatch(glmmadmb(mod.formula, segSample, family = 'gaussian'), error = function(e) NULL)
}
...
```{r setup_re_os}
fe.os.max <- MeanODBA ~ WindAngle * WindSpd
...
Based on AIC the top ranked FV random effects structure is (M|B). (1|B) had delta AIC of 1.806.

## Determine optimal fixed effects structure
Using the optimal random effects structure, determine best fitting fixed effects structure
```{r setup_fe_os}
# Formula of optimal random effects structure
re.os.optim <- MeanODBA ~ (WindSpd|DeployID)
# List to store fixed effects model formulae
me.os.form <- list(list(~ . + WindAngle * WindSpd, re.os.optim),
                list(~ . + WindAngle + WindSpd, re.os.optim),
...
modCl <- makeCluster(detectCores() - 1, type = 'FORK')
registerDoParallel(modCl)
me.os.mod <- foreach(f = me.os.form,
                     i = seq_along(re.os.form)) %dopar% {
  # Update 
  mod.formula <- update.formula(f[[2]], f[[1]])
  tryCatch(glmmadmb(mod.formula, segSample, family = 'gaussian'), error = function(e) NULL)
}
...
# Model comparison
```{r mod_comp}
# FV model: FlapRatio ~ Tailwind * Crosswind + (1|DeployID)
fv.mod <- me.fv.mod[[1]]
# FS model: FlapRatio ~ WindAngle + WindSpd + (1|DeployID)
fs.mod <- me.fs.mod[[2]]
# OV model: MeanODBA ~ Tailwind + Crosswind + (Tailwind|DeployID) + (Tailwind|Leg)
ov.mod <- me.ov.mod[[2]]
# OS model: MeanODBA ~ WindAngle * WindSpd + (WindSpd|DeployID)
os.mod <- me.os.mod[[1]]

AIC(fv.mod, fs.mod, ov.mod, os.mod)
```