...
vis.gam(ftc, plot.type = 'contour', type = 'response')
vis.gam(fam, plot.type = 'contour', type = 'response')
vis.gam(otc, plot.type = 'contour', type = 'response')
vis.gam(oam, plot.type = 'contour', type = 'response')

# Visualize both on a angle/speed plot
grain <- 20
model_predictions <- expand.grid(WindAngle = seq(0, pi, length.out = grain),
                                 WindSpd = seq(0, 13, length.out = grain)) %>%
...
ggplot(model_predictions, aes(WindAngle, WindSpd)) +
  ...
  stat_contour(aes(z = FTC)) +
  scale_fill_distiller(palette = 'RdBu')

ggplot(model_predictions, aes(WindAngle, WindSpd)) +
  geom_raster(aes(fill = FAM)) +
  stat_contour(aes(z = FAM)) +
  scale_fill_distiller(palette = 'RdBu')
```
