...
m <- gamm(FlapRatio ~ s(WindAngle), random = list(DeployID = ~1), family = gaussian(link = 'identity'), data = segSample)
segSample %>%
  mutate(mod.fit = predict(m$lme),
         mod.resid = resid(m$lme)) %>%
  ggplot(aes(x = DeployID, y = mod.resid)) +
  geom_boxplot()

segSample %>%
  mutate(mod.fit = predict(m$lme),
         mod.resid = resid(m$lme)) %>%
  ggplot(aes(x = FlapRatio, y = mod.fit)) +
  geom_point() +
  geom_smooth()
```
