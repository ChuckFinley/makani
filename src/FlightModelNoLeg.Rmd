...
segSample <- windSegments %>%
  ...
  sample_n(1) %>%
  ungroup %>%
  mutate(WindMag = sqrt(Tailwind^2 + Crosswind^2),
...
interp.seg <- with(segSample, interp(Tailwind, Crosswind, FlapRatio))
expand.grid(i = seq(interp.seg.in$x),
            j = seq(interp.seg.in$y)) %>% 
  mutate(Tailwind = sapply(i, function(a) interp.seg$x[a]),
         Crosswind = sapply(j, function(b) interp.seg$y[b]), 
         FlapRatio = mapply(function(a, b) interp.seg$z[a, b], i, j)) %>%
  ggplot(aes(x = Tailwind, y = Crosswind, fill = FlapRatio)) +
  geom_tile() +
  xlim(-12, 12) +
  ylim(0, 11) +
  scale_fill_distiller(palette = 'RdBu') +
  labs(title = 'Interpolated Flap Ratios')
...
```{r}
segSample %>%
  ggplot(aes(x = WindAngle, y = FlapRatio)) +
  geom_smooth(method = 'loess') +
  geom_point(aes(color = WindMag)) +
  scale_x_continuous(breaks = pi/4 *0:4,
                     labels = c('0', 'pi/4', 'pi/2', '3pi/4', 'pi'),
                     name = 'Tailwind  <---  Wind Angle  --->  Headwind')

interp.seg <- with(segSample, interp(WindAngle, WindMag, FlapRatio))
expand.grid(i = seq(interp.seg.in$x),
            j = seq(interp.seg.in$y)) %>% 
  mutate(WindAngle = sapply(i, function(a) interp.seg$x[a]),
         WindMag = sapply(j, function(b) interp.seg$y[b]), 
         FlapRatio = mapply(function(a, b) interp.seg$z[a, b], i, j)) %>%
  ggplot(aes(x = WindAngle, y = WindMag, fill = FlapRatio)) +
  geom_tile() +
  xlim(0, pi) +
  ylim(0, 14) +
  scale_x_continuous(breaks = pi/4 *0:4,
...
m.head <- lmer(FlapRatio ~ WindAngle + (1|DeployID), data = filter(segSample, WindAngle < pi/2))
summary(m.head)
m.head2 <- lmer(FlapRatio ~ WindAngle * Tailwind + (1|DeployID), data = filter(segSample, WindAngle < pi/2))
summary(m.head2)
AIC(m.head, m.head2)

m.tail <- lmer(FlapRatio ~ WindAngle + (1|DeployID), data = filter(segSample, WindAngle >= pi/2))
summary(m.tail)
m.tail2 <- lmer(FlapRatio ~ WindAngle * Tailwind + (1|DeployID), data = filter(segSample, WindAngle >= pi/2))
summary(m.tail2)
AIC(m.tail, m.tail2)

cardinality <- 30
predict.F <- Vectorize(function(t, c) {
  a <- atan2(c, t)
  m <- if(a < pi/2) m.head else m.tail
  df <- data.frame(WindAngle = a)
  predict(m, newdata = df, type = 'response', re.form = NA)
})
model.predictions <- expand.grid(WindAngle = seq(0, pi, length.out = cardinality),
                                 WindMag = seq(0, 14, length.out = cardinality)) %>%
  mutate(FlapRatio = predict.F(Tailwind, Crosswind))
...
wind.spd <- 5
data.frame(windAngle = seq(0, pi, length.out = 20)) %>%
  mutate(TailwindOut = wind.spd * cos(windAngle),
         ...
         In = predict(flight.model, 
                       ...
                       type = 'response'),
         Net = (Out + In) / 2) %>%
  gather(key = Direction, value = FlapRatio, Out:Net) %>%
  ggplot(aes(x = windAngle, y = FlapRatio, color = Direction)) +
  geom_line() +
  scale_x_continuous(breaks = pi/4 *0:4, 
                     labels = c('0', 'pi/4', 'pi/2', '3pi/4', 'pi'))
```
