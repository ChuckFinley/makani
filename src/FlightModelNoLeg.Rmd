...
interp.seg <- with(segSample, 
                   interp(Tailwind, Crosswind, FlapRatio, nx = 40, ny = 20))
...
interp.spd <- with(segSample, 
                   interp(Tailwind, Crosswind, meanSpd - Tailwind, nx = 40, ny = 20))
expand.grid(i = seq(interp.spd$x),
            j = seq(interp.spd$y)) %>% 
  mutate(Tailwind = sapply(i, function(a) interp.spd$x[a]),
         Crosswind = sapply(j, function(b) interp.spd$y[b]), 
         AirSpeed = mapply(function(a, b) interp.spd$z[a, b], i, j)) %>%
  ggplot(aes(x = Tailwind, y = Crosswind, fill = AirSpeed)) +
  geom_tile() +
  geom_abline(intercept = 0, slope = c(-1, 1)) +
  scale_x_continuous(breaks = seq(-12, 12, by = 3)) +
  scale_y_continuous(breaks = seq(0, 12, by = 3)) +
  ...
  coord_fixed(xlim = c(-12, 12),
              ylim = c(0, 12)) +
  labs(title = 'Interpolated Air Speeds')
