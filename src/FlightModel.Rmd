...
p3 <- ggplot(segSample, aes(x = WindAngle, fill = Leg)) +
  geom_histogram(binwidth = pi/16, boundary = 0) + 
  angle_scale +
  scale_fill_manual(values = c('#ef8a62', '#67a9cf'), guide = FALSE) +
  labs(y = 'frequency') +
...
# Significant difference in wind angle between outbound and inbound?
circular::watson.wheeler.test(WindAngle ~ Leg, segSample)
...
save(ftc, fam, otc, oam, file = '../data/out/Models/EnergyModels.Rdata')
...
save(SgTCModel, file = '../data/out/Models/SgTCModel.RData')
